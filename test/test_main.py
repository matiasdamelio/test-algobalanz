...
def test_home():
    ...
    assert b"Welcome FastAPI Web Starter" in response.content
...
def test_page_about():
    response = client.get(
        "/page/about", headers={"content-type": "text/html; charset=utf-8"}
    )
    assert response.status_code == 200
...
def test_unsplash():
    response = client.get(
        "/unsplash", headers={"content-type": "text/html; charset=utf-8"}
    )
    assert response.status_code == 200
...
def test_two_forms():
    response = client.get(
        "/twoforms", headers={"content-type": "text/html; charset=utf-8"}
    )
    assert response.status_code == 200
